        ...
        <div className="flex flex-col gap-2 sm:gap-3">
            <div className="flex flex-col sm:flex-row justify-between sm:content-center gap-2 sm:gap-0">
                <div className="flex flex-row gap-2 sm:gap-2.5 items-center">
                    ...
                    <h3 className="text-base sm:text-lg font-bold text-[var(--shared-label-primary)]">{company}</h3>
                </div>
                <div className="flex items-center sm:items-start">
                    <p className="text-sm sm:text-base font-semibold text-[var(--shared-label-secondary)]">{duration}</p>
                </div>
            </div>
            <p className="text-sm sm:text-base font-semibold text-[var(--shared-label-primary)]">{title}</p>
            <p className="text-sm sm:text-base font-normal text-[var(--shared-label-secondary)]">{body}</p>
        </div>
    );
}